...
from util.edamodel import local_search, VWH
import math

def get_reproduction(opt_name='VWH_UE', **kwargs):
    # check opt_name
    if opt_name not in ['VWH','VWH_UE', 'CoDE']:
        raise ValueError("The opt_name {} is not supported.".format(opt_name))

    if opt_name == 'VWH':
        reproduction = VWH_Local_Reproduction(**kwargs)
    elif opt_name == 'VWH_UE':
        reproduction = VWH_Local_Reproduction_unevaluate(**kwargs)
    elif opt_name == 'CoDE':
        reproduction = CoDE_Reproduction(**kwargs)
    else:
        raise ValueError("The opt_name {} is not supported.".format(opt_name))
    return reproduction
